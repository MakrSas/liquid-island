...
struct IslandBody: View {
    ...
    private var content: some View {
        if let hudEvent {
            ...
            .opacity(swipeFade)
            // Переход срабатывает только когда карточка сменяет плашку:
            // при раскрытии и сворачивании ветка та же самая, и вьюха не
            // подменяется, а перетекает.
            .transition(.softFade(scale: 0.94, offsetY: 0))
        } else {
