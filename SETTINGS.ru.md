...
| `pausedArtworkScale` | 0.5…1 | 0.82 | Насколько поджимается обложка на паузе |
...
| `collapseForSystemEvents` | да/нет | да | Сворачивать раскрытый остров, когда приходит системное событие |
| `restoreAfterSystemEvent` | да/нет | да | Возвращать его обратно, когда плашка отработала |
| `dimArtworkWhenPaused` | да/нет | да | Приглушать обложку сразу, как воспроизведение встало |
| `hideWhenPaused` | да/нет | да | Убирать карточку, если пауза затянулась |
| `hideWhenPausedAfter` | 5…180 с | 30 | Сколько ждать |
| `dotsPlacement` | `inside` / `below` | `inside` | Где точки активностей: внутри карточки у нижней кромки или отдельной капсулой под островом |
| `dotSize` / `dotSpacing` | 3…10 | 5 / 6 | Сами точки. Ширина капсулы считается по ним |
| `dotsCapsuleHeight` / `dotsCapsulePadding` / `dotsCapsuleGap` | pt | 20 / 12 / 7 | Капсула под островом |
